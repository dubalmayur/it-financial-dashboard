...
import { generateAIInsights, companies, quarters, getCompanyData } from '../data/sampleData';
...
export default function AIInsights({ company, quarter }) {
  ...
  const insights = generateAIInsights(company, quarter);
  ...
  return (
    <div>
      <div className="page-header">
        <div>
          <div className="page-title">AI Insights — {company} · {quarter}</div>
          <div className="page-subtitle">Automated intelligence powered by financial analytics engine</div>
        </div>
      ...
      <div className="chart-card" style={{ marginBottom: 20 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          ...
          <div style={{ fontSize: 11, color: 'var(--text-secondary)', background: 'var(--bg-primary)', padding: '4px 10px', borderRadius: 4 }}>
      ...
      {insights.length === 0 && (
        <div style={{ textAlign: 'center', padding: 60, color: 'var(--text-secondary)' }}>
          ...
          <div>Select a company and quarter to generate AI insights</div>
        </div>
      )}
      ...
      <div style={{ marginTop: 24, padding: '12px 16px', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 8, fontSize: 11, color: 'var(--text-muted)', lineHeight: 1.6 }}>
        <strong style={{ color: 'var(--text-secondary)' }}>Disclaimer:</strong> These AI-generated insights are based on quantitative financial data analysis only. They are for analytical purposes and do not constitute investment advice. Past performance is not indicative of future results. Always consult qualified financial advisors before making investment decisions.
      </div>
    </div>
  );
}
